...
import {
  FileText, Search, FilePlus, Filter,
  CheckCircle2, Clock, Send, AlertTriangle,
  Package, XCircle, ChevronRight,
} from "lucide-react";
...
export default function PrescriptionsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <PageHeader
        title="Prescriptions"
        subtitle={`${mockPrescriptions.length} prescriptions total`}
      ...
      <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
        {STATUS_FILTERS.map((f) => {
          const Icon = f.icon;
          const count = f.value === "All" ? mockPrescriptions.length : mockPrescriptions.filter((r) => r.status === f.value).length;
          return (
            <button
              key={f.value}
