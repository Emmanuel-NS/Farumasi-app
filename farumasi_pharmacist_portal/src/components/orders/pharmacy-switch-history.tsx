      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
        ...
        {assignments.length === 1 ? (
          <p className="text-sm text-slate-600">
            Assigned to{" "}
            <span className="font-semibold text-slate-800">{assignments[0].provider_name}</span>{" "}
            since {formatDateTime(assignments[0].assigned_at)}. No pharmacy switches yet.
          </p>
        ) : (
          <p className="text-sm text-slate-500">No pharmacy switches recorded for this order.</p>
        )}
      </div>
    );
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            ...
            {amountPaidSnapshot != null && (
              <> · original payment {formatPrice(amountPaidSnapshot)} RWF</>
            )}
          </p>
        </div>
      </div>
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="text-sm font-bold text-slate-900 truncate">{row.provider_name}</p>
                  {row.is_current && (
              ...
              <p className="text-xs text-slate-500 mt-2">
                Quote at assignment: {formatPrice(row.subtotal)} RWF subtotal ·{" "}
                {formatPrice(row.net_partner_amount)} RWF net to partner
              </p>
                ...
                <div className="mt-3 pt-3 border-t border-amber-200/80">
                  ...
                  {switchActor && (
                    <p className="text-xs text-amber-800/90 mt-1 flex items-center gap-1 flex-wrap">
                      ...
                      {audit?.new_total != null && audit?.amount_paid != null && (
                        <>
                          {" · "}
                          New quote {formatPrice(audit.new_total)} RWF
                          {audit.new_total < audit.amount_paid && (
                            <> (patient paid {formatPrice(audit.amount_paid)} RWF — no refund)</>
                          )}
                        </>
                      )}
                    </p>
