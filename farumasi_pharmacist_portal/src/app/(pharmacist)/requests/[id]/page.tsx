...
export default function RequestDetailPage() {
  ...
  // Insurance coverage (only for prescription orders)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (data.status === "reviewed" && data.items.length > 0) {
        ...
        // Restore insurance info if previously set
  ...
  const sendCartToPatient = async () => {
    ...
    try {
      ...
      // 3. Save cart-level notes, insurance info, and mark as "reviewed" (cart ready for patient)
            ...
            {rx.insurance_provider && (
              <div className="mt-2 flex items-center gap-2 bg-green-50 border border-green-100 rounded-xl px-3 py-2">
                ...
                <div className="text-xs text-green-700">
                  <span className="font-bold">{rx.insurance_provider}</span> insurance applied
                  {rx.insurance_discount_pct ? ` — ${rx.insurance_discount_pct}% discount` : ""}
                </div>
              </div>
            )}
          </div>
