"use client";
// TipTap nested dependency versions produce duplicate @tiptap/core types in strict builds.
// @ts-nocheck
...
interface RichEditorProps {
  ...
  /** Remount / resync when switching documents */
  editorKey?: string;
...
function youtubeEmbedHtml(videoId: string): string {
  return `<div class="video-embed" style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;max-width:100%;margin:1rem 0;"><iframe src="https://www.youtube.com/embed/${videoId}" title="YouTube video" style="position:absolute;top:0;left:0;width:100%;height:100%;border:0;border-radius:0.75rem;" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`;
}
...
function YoutubeDialog({ onConfirm, onCancel }: {
  onConfirm: (embedHtml: string) => void; onCancel: () => void;
}) {
  const [url, setUrl] = useState("");
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 flex flex-col gap-4">
        <p className="font-bold text-slate-900 text-base">Embed YouTube Video</p>
        <input
          autoFocus type="url" value={url}
          onChange={(e) => setUrl(e.target.value)}
        ...
        />
        {videoId && (
          <p className="text-xs text-emerald-600">Valid video ID: {videoId}</p>
        )}
        <div className="flex justify-end gap-2">
          <button type="button" onClick={onCancel} className="px-4 py-2 rounded-xl text-sm text-slate-600 hover:bg-slate-100">Cancel</button>
          <button
            type="button"
            onClick={() => videoId && onConfirm(youtubeEmbedHtml(videoId))}
            disabled={!videoId}
            className="px-4 py-2 rounded-xl text-sm font-semibold bg-farumasi-600 text-white hover:bg-farumasi-700 disabled:opacity-40"
          >
            Insert
...
export function RichEditor({
  value, initialContent, editorKey, onChange, placeholder = "Start writing...",
  ...
  }, [editor, resolvedValue, editorKey]);
    ...
    <>
      ...
      {youtubeDialog && (
        <YoutubeDialog
          ...
          onCancel={() => setYoutubeDialog(false)}
        />
      )}
      ...
      <div className={cn(
        ...
        <div className="flex flex-wrap items-center gap-1 px-2.5 py-1.5 bg-slate-50/60 border-b border-slate-100">
          ...
          <Btn title="Embed YouTube" onClick={() => setYoutubeDialog(true)}>
          ...
          <Btn title="Delete table" onClick={() => editor.chain().focus().deleteTable().run()}>
            <span className="text-[10px] font-bold text-red-500">T×</span>
          </Btn>
        ...
        <EditorContent
          ...
          className={cn(
            ...
            "[&_th]:border [&_th]:border-slate-200 [&_th]:bg-slate-50 [&_th]:px-2 [&_th]:py-1.5 [&_th]:text-left",
