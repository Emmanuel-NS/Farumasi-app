...
function describeEntry(entry: OrderActivityEntry): string {
  if (entry.action === "order.pharmacy_reassigned" && entry.new_value && typeof entry.new_value === "object") {
    ...
    const from = v.previous_provider_name ?? "previous partner";
    const to = v.new_provider_name ?? "new partner";
    return `Patient switched from ${from} to ${to}`;
  }
  if (entry.action === "order.dispatch_confirmed") {
    return "Dispatch confirmed with batch traceability records";
  }
...
export function OrderActivityTimeline({ entries }: { entries: OrderActivityEntry[] }) {
  if (entries.length === 0) {
    return <p className="text-sm text-muted-foreground py-2">No activity recorded for this order yet.</p>;
  }
      ...
      {entries.map((entry, i) => (
        <li key={entry.id} className="flex gap-3">
          ...
          <div className="flex-1 pb-3">
            <p className="text-sm font-semibold text-slate-800 capitalize">{describeEntry(entry)}</p>
