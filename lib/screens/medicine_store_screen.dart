...
class _MedicineStoreScreenState extends State<MedicineStoreScreen> with SingleTickerProviderStateMixin {
  // Search & Filter State
  String _searchQuery = '';
  ...
  List<Medicine> get _filteredMedicines {
    return dummyMedicines.where((m) {
      ...
      final matchesCategory = _selectedCategories.isEmpty || _selectedCategories.contains(m.category);
      final matchesPrice = m.price >= _priceRange.start && m.price <= _priceRange.end;
  ...
  List<Medicine> get _popularMedicines => dummyMedicines.where((m) => m.isPopular).toList();

  List<String> get _categories => dummyMedicines.map((e) => e.category).toSet().toList();

  IconData _getCategoryIcon(String category) {
    switch (category) {
      case 'Pain Relief': return Icons.healing;
      case 'Antibiotics': return Icons.science;
      case 'Vitamins': return Icons.wb_sunny;
      case 'Cold & Flu': return Icons.snowing;
      case 'Skincare': return Icons.face_retouching_natural;
      case 'Sexual Health': return Icons.favorite;
      case 'Mobility Aids': return Icons.accessible;
      case 'Mother & Baby': return Icons.child_friendly;
      case 'Devices': return Icons.monitor_heart;
      case 'First Aid': return Icons.medical_services;
      case 'Chronic Care': return Icons.medication_liquid;
      case 'Nutrition': return Icons.fitness_center;
      case 'Herbal Medicines': return Icons.spa;
            ...
            child: Column(
              ...
              children: [
                Row(
                  ...
                  children: [
                    Text('Filters', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
                    TextButton(
                      child: Text('Reset'),
                      onPressed: () {
                        setModalState(() {
                  ...
                  child: ListView(
                    children: [
                       Text('Sort By', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        Wrap(
                          spacing: 8,
                          children: ['Popularity', 'Price Low-High', 'Price High-Low', 'Name'].map((sort) {
                            return ChoiceChip(
                              label: Text(sort),
                        ...
                        Wrap(
                         ...
                         SizedBox(height: 16),
                        Text('Price Range: ${_priceRange.start.round()} - ${_priceRange.end.round()} RWF', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        RangeSlider(
                          values: _priceRange,
                          min: 0,
                          max: 50000,
                          divisions: 50,
                          labels: RangeLabels('${_priceRange.start.round()} RWF', '${_priceRange.end.round()} RWF'),
                          onChanged: (values) {
        ...
        slivers: [
          ...
          SliverPersistentHeader(
            ...
            delegate: _StickyHeaderDelegate(
              height: _showCategories ? 190 : 92, // Increased height for padding
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    color: Colors.green,
                    padding: const EdgeInsets.fromLTRB(16, 24, 16, 4), // Increased top padding
                    child: Row(
                      children: [
                        // Sticky Header Logo (Mini Version)
                        FarumasiLogo(size: 40, color: Colors.green, onDark: true),
                        SizedBox(width: 10),
                        ...
                        SizedBox(width: 8),
                        IconButton(
                          icon: Icon(_showCategories ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, color: Colors.white),
                          onPressed: () => setState(() => _showCategories = !_showCategories),
                          tooltip: _showCategories ? "Hide Categories" : "Show Categories",
                        )
                      ],
                    ),
                  ),
                          ...
                          child: Column(
                            children: [
                              AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  color: _selectedCategories.contains(cat) ? Colors.green : Colors.grey.shade100,
                                  shape: BoxShape.circle,
                                  border: Border.all(color: _selectedCategories.contains(cat) ? Colors.green : Colors.grey.shade300),
                                ),
                                child: Icon(_getCategoryIcon(cat), color: _selectedCategories.contains(cat) ? Colors.white : Colors.green, size: 28),
                              ),
                              SizedBox(height: 8),
                              Text(
                                cat, 
                                style: TextStyle(
                                  fontSize: 11, 
                                  fontWeight: _selectedCategories.contains(cat) ? FontWeight.bold : FontWeight.w500,
                  ...
                  Divider(height: 1, thickness: 1, color: Colors.green.shade800),
                ],
              ),
            ),
          ),
             ...
             SliverToBoxAdapter(
              child: Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text('Popular Today', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  ],
                ),
              ),
            ),
                      ...
                      child: MedicineItem(
                        ...
                        onTap: () {
                          if (med.requiresPrescription) {
                           ...
                           final isAdded = StateService().cartItems.any((item) => item.medicine.id == med.id);
                           if (isAdded) {
                        ...
                        onAboutTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => MedicineDetailScreen(medicine: med))),
                      ),
                    );
                  },
                ),
          ...
          SliverToBoxAdapter(
            child: Padding(
              padding: const EdgeInsets.fromLTRB(16, 16, 16, 8),
              child: Text(
                _searchQuery.isNotEmpty ? 'Search Results' : (_selectedCategories.isNotEmpty ? 'Filtered Results' : 'Explore Medicines'), 
